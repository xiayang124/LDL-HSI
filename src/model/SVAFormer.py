...
import random
from engine.config import SVASetting
...
class SVAFormer(nn.Module):
    def __init__(self, params: SVASetting):
        super(SVAFormer, self).__init__()
        num_classes = params.get("num_classes", 16)
        self.patch_size = patch_size = params.get("patch_size", 13)
        self.serve_patch_size = params.get("serve_patch_size", 13)
        print("serving_patch_size=%s" % self.serve_patch_size)
        self.spectral_size = params.get("spectral_size", 200)

        depth = params.get("depth", 1)
        heads = params.get("heads", 8)
        mlp_dim = params.get("mlp_dim", 8)
        ...
        self.mask_pct = params.get("mask_pct", 50)
        conv2d_out = dim
        dim_heads = dim
...
#            return patch
    ...
    def encoder_block(self, x):
        ...
        if self.use_mask:
            if self.training:
                ...
                cur_mask = self.random_mask(self.patch_size).to(self.device)
            else:
                # serving
                cur_mask = self.get_serving_mask().to(self.device)
        else:
            cur_mask = None 
    ...
    def forward(self, x):
        ...
        '''
        logit_x, image_token_x = self.encoder_block(x)
        return  logit_x, image_token_x